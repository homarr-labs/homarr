...
export const yourSpotifyStatsProvider = {
  ...
  async fetchAsync(context) {
    const [songsResponse, timeResponse, artistsResponse] = await Promise.all([
      context.requestAsync(endpointPath("songs_per"), { signal: context.signal }),
      context.requestAsync(endpointPath("time_per"), { signal: context.signal }),
      context.requestAsync(endpointPath("different_artists_per"), { signal: context.signal }),
    ]);
