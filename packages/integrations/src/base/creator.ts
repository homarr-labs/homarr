...
import { clientAuthentication } from "../http-auth";
...
export async function getIntegrationHttpAuthenticationAsync(integration: IntegrationInput & { kind: IntegrationKind }) {
  if (!isHttpIntegrationKind(integration.kind)) throw new Error("This integration does not use HTTP");
  return clientAuthentication(() => createIntegrationAsync(integration));
}
