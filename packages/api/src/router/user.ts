...
export const userRouter = createTRPCRouter({
  ...
  selectable: protectedProcedure
    .input(z.object({ excludeExternalProviders: z.enum(["true", "false"]).default("false") }).optional())
    ...
    .query(({ ctx, input }) => {
      return ctx.db.query.users.findMany({
        ...
        where: input?.excludeExternalProviders === "true" ? eq(users.provider, "credentials") : undefined,
      });
    }),
