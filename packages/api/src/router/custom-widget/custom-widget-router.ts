...
const MAX_SUB_FETCH_BODY_BYTES = 2 * 1024 * 1024;

function assertSafeFetchUrl(url: URL) {
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new TRPCError({ code: "BAD_REQUEST", message: `Blocked scheme: ${url.protocol}` });
  }

  const hostname = url.hostname.replace(/^\[|\]$/g, "");
  if (BLOCKED_HOSTNAMES.has(hostname.toLowerCase())) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "Blocked hostname" });
  }

  if (PRIVATE_IP_RANGES.some((re) => re.test(hostname))) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "Blocked private/internal IP" });
  }
}
...
export const customWidgetRouter = createTRPCRouter({
  ...
  subFetch: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const parentUrl = validateUrl(definition.url);
      if (url.hostname !== parentUrl.hostname) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: `Sub-fetch URL must be on the same host as the widget (${parentUrl.hostname})`,
        });
      }
      ...
      try {
        ...
        if (response.status >= 300 && response.status < 400) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Sub-fetch redirects are not followed" });
        }

        if (!response.ok) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `Sub-fetch returned ${response.status}: ${response.statusText}`,
          });
        }

        const contentType = response.headers.get("content-type") ?? "";
        if (!contentType.includes("application/json") && !contentType.includes("+json")) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Sub-fetch response is not JSON" });
        }

        const text = await response.text();
        if (text.length > MAX_SUB_FETCH_BODY_BYTES) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Sub-fetch response too large" });
        }

        return JSON.parse(text) as unknown;
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        logger.error("SubFetch failed", { url: input.url, error });
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: error instanceof Error ? error.message : "Sub-fetch failed",
        });
      } finally {
        clearTimeout(timeout);
      }
