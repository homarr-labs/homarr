...
describe("SelectableCard", () => {
  ...
  it("prioritizes title without wrapping and constrains topRight badge with ellipsis", async () => {
    await act(async () =>
      root.render(
        <MantineProvider>
          <SelectableCard title="Very Long Title Name That Should Not Wrap" topRight={<Badge>Category Badge</Badge>} />
        </MantineProvider>,
      ),
    );
    ...
    // The title text is rendered with whiteSpace: nowrap and fw: 700
    ...
    // Check that custom ReactNode title is supported
    await act(async () =>
      root.render(
        <MantineProvider>
          <SelectableCard
  ...
  it("supports keyboard activation via Enter and Space", async () => {
    ...
    // Native buttons fire click event on Enter / Space keydown/keyup
    await act(async () => {
