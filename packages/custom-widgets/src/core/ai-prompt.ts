...
Core schema/runtime/safety/lifecycle rules are bundled; do not call customWidget_getSkill. Unknown edit: call customWidget_list once, then get the exact widget. Persisted follow-up: pass its id as definitionId to previewCreate. Preserve sources unless explicitly changed; style-only edits preserve requests/options. Never turn an edit into a create. Load required references and uncertain components with customWidget_findComponents, then at most eight needed component documents in one getComponents batch. Reuse loaded context/contextAlreadyLoaded and follow nextStep. Inventory operations, paths, states, and count.
