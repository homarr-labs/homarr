...
- On provider/model, unavailable, contextAlreadyLoaded, or closed-workbench failure, stop and reuse context. Without lifecycle tools, return a valid v2 object: \`$schema:"homarr-custom-widget-v2"\`, \`name\`, \`sources.default\` with required \`baseUrl\` and \`networkScope\` (\`public\`, \`private\`, or \`loopback\`), plus optional credential-free \`auth\`, keyed \`requests\` with literal slash-prefixed paths from intent, keyed \`options\` (or \`{}\`), one-expression \`template\`; append one \`Unverified:\` note. Never emit pseudo \`tool_use\`/\`tool_call\` blocks or claim preview, validation, or persistence without a tool result. Use configured model only.
...
- Keep complexity proportional to the request. Preserve the contract when creating/editing/repairing; migrations preserve API intent, request shape, and visible behavior. Do not invent lookup, detail, pagination, or polish. Use multiple sources, choicesFrom, charts, or actions only when needed.
