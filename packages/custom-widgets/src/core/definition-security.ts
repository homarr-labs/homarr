...
export function redactCustomWidgetCredentialLiterals(value: string): string {
  return value
    ...
    .replace(
      authorizationSchemePattern,
      (match, prefix: string, scheme: string, separator: string, quote: string, candidate: string) => {
        if (isHarmlessCustomWidgetCredentialSetting(candidate)) return match;
        return `${prefix}${scheme}${separator}${quote}[REDACTED]`;
      },
    )
