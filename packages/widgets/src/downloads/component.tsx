...
export default function DownloadClientsWidget({
  ...
  const [quickFilters, setQuickFilters] = useState<FilterSelect>({ names: [], statuses: [] });
  const availableStatuses = useMemo<FilterSelect["statuses"]>(() => {
    ...
    setQuickFilters(({ names, statuses: prevStatuses }) => {
      return { names, statuses: prevStatuses.filter((status) => statuses.includes(status)) };
    });
    return statuses;
  }, [currentItems]);
      ...
      currentItems
        ...
        .flatMap((pair) =>
          ...
          pair.data.items
            ...
            .filter(
              ({ state }) =>
                (quickFilters.names.length === 0 || quickFilters.names.includes(pair.integration.name)) &&
...
interface ClientsControlProps {
  ...
  setFilters: (filters: FilterSelect) => void;
  availableStatuses: FilterSelect["statuses"];
      ...
      <Popover withinPortal={false} offset={0}>
        ...
        >
          <Stack gap="var(--space-size)" align="center" pb="var(--space-size)">
            ...
            <Chip.Group multiple value={filters.names} onChange={(names) => setFilters({ ...filters, names })}>
