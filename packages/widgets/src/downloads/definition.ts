      ...
      (factory) => ({
        columns: factory.multiSelect({
          ...
          options: columnsList.map((value) => ({
            value,
            label: (t: (s: string) => string) => t(`widget.downloads.items.${value}.columnTitle`),
          })),
          searchable: true,
        }),
        enableRowSorting: factory.switch({ defaultValue: false }),
        defaultSort: factory.select({
          defaultValue: "type" as const,
        ...
        showCompletedUsenet: factory.switch({ defaultValue: true }),
        showCompletedTorrent: factory.switch({ defaultValue: true }),
        showCompletedHttp: factory.switch({ defaultValue: true }),
        activeTorrentThreshold: factory.number({ validate: z.number().min(0), defaultValue: 0, step: 1 }),
        categoryFilter: factory.multiText({ defaultValue: [] as string[], validate: z.string() }),
        filterIsWhitelist: factory.switch({ defaultValue: false }),
        applyFilterToRatio: factory.switch({ defaultValue: true }),
        limitPerIntegration: factory.number({ defaultValue: 50, validate: z.number().min(1), withDescription: true }),
      }),
      {
