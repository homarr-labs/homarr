...
export default function HealthMonitoringWidget({ options, integrationIds }: WidgetComponentProps<"healthMonitoring">) {
  const [healthData] = clientApi.widget.healthMonitoring.getHealthStatus.useSuspenseQuery(
    {
      integrationIds,
      maxElements: Number(options.pointDensity),
      pointCount: Number(options.pointDensity),
    },
    {
  ...
  return (
    <Tabs defaultValue={integrationIds[0]} h="100%" display="flex" dir="vertical" style={{ flexDirection: "column" }}>
      ...
      {healthData.map(({ integration, healthInfo: { data } }) => {
        return (
          ...
          >
            {options.systemInfo && (
              <Box flex="1 1 auto" p="1cqw">
                <Stack gap="1cqmin" align="center">
                  <Text fz="10cqmin">{data.system.name}</Text>
                  <Text fz="5cqmin">{formatUptime(data.system.uptime, t)}</Text>
                </Stack>
              </Box>
            )}
            {options.cpu && <GridAreaChart columns={Number(options.cpuColumns)} data={data} series={["cpu"]} />}
            {options.memory && data.system.type === "single" && (
              <GridAreaChart columns={1} data={data} series={["memory"]} />
            )}
            {options.network && <GridAreaChart columns={1} data={data} series={["networkUp", "networkDown"]} />}
            {options.fileSystem && (
              <StorageEntries columns={1} fahrenheit={options.fahrenheit} storages={data.storage} />
            )}
          </Tabs.Panel>
        );
      })}
...
const CustomTooltip = ({ payload }: CustomTooltipProps) => {
  ...
  return (
    <Box bg="var(--background-color)" p="1cqmin" style={{ borderRadius: "3cqmin" }}>
      ...
      {filteredData.map(({ dataKey, payload: subPayload }) => {
        ...
        return (
          <Text key={dataKey as string} fz="6cqmin">
        ...
        return (
          ...
          >
            <Group w="100%" justify="space-between" gap="1cqmin">
              <Text fz="6cqmin">{storage.name}</Text>
              {storage.temp && (
                <Group>
                  <Text fz="6cqmin">{temperatureFormatter(storage.temp, fahrenheit)}</Text>
                </Group>
              )}
              <Text fz="6cqmin">{`${humanFileSize(storage.used)}/${humanFileSize(storage.size)}`}</Text>
            </Group>
            <Progress value={percentage} h="5cqmin" m="1cqmin" color={progressColor(percentage)} />
          </Stack>
        );
      })}
    </Box>
