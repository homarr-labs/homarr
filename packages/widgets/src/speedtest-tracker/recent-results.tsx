...
export function RecentResultsSection({ results }: { results: SpeedtestTrackerResult[] }) {
...
function SpeedHistoryChart({ results, height }: { results: SpeedtestTrackerResult[]; height: number }) {
  ...
  // Custom tick renderer passed to recharts' `tick` prop. We use raw SVG (<g>/<text>)
  // rather than a string formatter because midnight ticks need two lines of text: the
  // time on the first line and the date on the second. recharts' built-in `tickFormatter`
  // only supports a single string, so a custom SVG structure is the only way to render
  // multi-line tick labels here.
