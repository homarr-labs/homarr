...
export const serverDefinition = {
  createOptions() {
    return optionsBuilder.from(
      (factory) => ({
        customTitleToggle: factory.switch({ defaultValue: false, withDescription: true }),
        customTitle: factory.text({ defaultValue: "" }),
        is24HourFormat: factory.switch({ defaultValue: true, withDescription: true }),
        ...
        showDate: factory.switch({ defaultValue: true }),
        ...
        customTimeFormat: factory.text({ defaultValue: "", withDescription: true }),
        customDateFormat: factory.text({ defaultValue: "", withDescription: true }),
      }),
      {
        customTitle: { shouldHide: (options) => !options.customTitleToggle },
        timezone: { shouldHide: (options) => !options.useCustomTimezone },
        dateFormat: { shouldHide: (options) => !options.showDate },
      },
    );
  },
};
