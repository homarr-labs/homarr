...
describe("Custom JSX custom widgets", () => {
  test("reliably creates, previews, and edits a customJsx widget", async () => {
    ...
    try {
      for (const attempt of [1, 2, 3]) {
        ...
        const widgetName = `E2E Custom JSX ${attempt}`;

        try {
          await loginAsync({ page, baseUrl, credentials: adminCredentials, destination });
          await expect(page.getByRole("textbox", { name: "Name" })).toBeVisible({ timeout: 30_000 });
          await page.getByRole("textbox", { name: "Name" }).fill(widgetName);
          ...
          const editNavigation = page.waitForURL((url) => url.pathname.startsWith("/manage/custom-widgets/edit/"), {
            timeout: 15_000,
            waitUntil: "domcontentloaded",
          });
          await page.getByRole("button", { name: "Create" }).first().click();
          await editNavigation;
          await expect(page.getByText(`Widget "${widgetName}" created successfully.`)).toBeVisible({
            timeout: 15_000,
          });
          ...
          await expect(page.getByText(`Widget "${widgetName}" updated successfully.`)).toBeVisible({
            timeout: 15_000,
          });
          await expect(page.getByLabel("JSX Template")).toHaveValue(updatedTemplate);
        } finally {
