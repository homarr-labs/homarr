...
for key,seconds in [('impact',61)]:
 ...
 assert stream['width']==1920 and stream['height']==1080 and int(stream['nb_frames'])==seconds*60
 assert abs(float(info['format']['duration'])-seconds)<.1
 (r/(key+'-metadata.json')).write_text(json.dumps(info,indent=2))
 subprocess.run(['ffmpeg','-y','-v','error','-i',str(v),'-vf','fps=1/4,scale=480:-1,tile=4x4','-frames:v','1',str(r/(key+'-review.jpg'))],check=True)
