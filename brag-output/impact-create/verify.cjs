...
const sourceChecks=[];
for(const scene of ['intro','workshop','integrations','custom','assistant','ending']){
 ...
 if(/ultimate (homarr )?update|your metrics|because i.m nice/i.test(result.text)||result.hasHalo)throw Error('Removed material remains in '+scene);
...
for(const [name,minTime,wait] of [['Intro',4,4500],['32 new integrations',18,4500],['Create a Custom Widget',28,2600],['Assistant · BYOK + free usage',42,4500],['Customized header',93,3600],['Ending',104.5,1800]]){
...
fs.writeFileSync(__dirname+'/verification.json',JSON.stringify({intro,integrations,sourceChecks,results,errors},null,2));console.log({intro,integrations,sourceChecks,results,errors});await b.close();if(errors.length)process.exit(1);
