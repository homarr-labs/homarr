...
describe("assistant-ui AI SDK v7 transport", () => {
  ...
  test("streams reasoning emitted after a completed tool call before the final answer", async () => {
    const model = new MockLanguageModelV4({
      doStream: [
        {
          stream: simulateReadableStream({
        ...
        },
        {
          stream: simulateReadableStream({
            chunks: [
              { type: "stream-start" as const, warnings: [] },
              { type: "reasoning-start" as const, id: "post-tool-reasoning" },
              {
                type: "reasoning-delta" as const,
                id: "post-tool-reasoning",
                delta: "Checking the returned value.",
              },
              { type: "reasoning-end" as const, id: "post-tool-reasoning" },
              { type: "text-start" as const, id: "answer" },
    ...
    const result = streamText({
      model,
      prompt: "Use the tool and explain the result.",
      tools: {
        test_lookup: tool({
          inputSchema: jsonSchema({
            type: "object",
    ...
    const transport = new AssistantChatTransport({
      api: "https://homarr.test/api/assistant/chat",
      fetch: () => Promise.resolve(result.toUIMessageStreamResponse({ sendReasoning: true })),
    });
    const stream = await transport.sendMessages({
      trigger: "submit-message",
      chatId: "thread-1",
      messageId: undefined,
      messages: [{ id: "user-1", role: "user", parts: [{ type: "text", text: "Run the tool" }] }],
      abortSignal: undefined,
    });

    const snapshots: string[] = [];
    for await (const message of readUIMessageStream({ stream })) snapshots.push(JSON.stringify(message));

    const reasoningSnapshotIndex = snapshots.findIndex((snapshot) => snapshot.includes("Checking the returned value."));
    const answerSnapshotIndex = snapshots.findIndex((snapshot) => snapshot.includes("Done."));
    expect(snapshots.join("\n")).toContain("Checking the returned value.");
    expect(reasoningSnapshotIndex).toBeGreaterThanOrEqual(0);
    expect(answerSnapshotIndex).toBeGreaterThan(reasoningSnapshotIndex);
  });
});
