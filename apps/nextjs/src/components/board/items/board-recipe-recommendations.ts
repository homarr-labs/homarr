...
export const getBoardRecipeRecommendations = ({
  ...
  return widgetKinds
    ...
    .flatMap((widgetKind) => {
      ...
      const matches = supportedIntegrations.filter((integrationKind) => availableKinds.has(integrationKind));
      ...
      return [
        {
          ...
          score: (preferredMatch ? 1_000 : 0) + matches.length * 10 - supportedIntegrations.length,
        },
      ];
    })
