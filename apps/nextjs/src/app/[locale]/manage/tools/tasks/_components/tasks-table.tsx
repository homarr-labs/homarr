...
const cronExpressions = [
	{
		value: "*/5 * * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.seconds", { interval: 5 }),
	},
	{
		value: "*/10 * * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.seconds", { interval: 10 }),
	},
	{
		value: "*/20 * * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.seconds", { interval: 20 }),
	},
	{
		value: "* * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.minutes", { interval: 1 }),
	},
	{
		value: "*/5 * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.minutes", { interval: 5 }),
	},
	{
		value: "*/10 * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.minutes", { interval: 10 }),
	},
	{
		value: "*/15 * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.minutes", { interval: 15 }),
	},
	// Every hour
	{
		value: "0 * * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.hours", { interval: 1 }),
	},
	// Every two hours
	{
		value: "0 */2 * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.hours", { interval: 2 }),
	},
	// Every four hours
	{
		value: "0 */4 * * *",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.hours", { interval: 4 }),
	},
	// Every midnight
	{
		value: "0 0 * * */1",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.midnight"),
	},
	{
		value: "0 0 * * 1",
		label: (t: TranslationFunction) =>
			t("management.page.tool.tasks.interval.weeklyMonday"),
	},
...
const createColumns = (
	...
	updateIntervalMutation: ReturnType<
		typeof clientApi.cronJobs.updateJobInterval.useMutation
	>,
	...
	loadingStates: Map<
		string,
		{ toggle: boolean; trigger: boolean; interval: boolean }
	>,
...
export const TasksTable = ({ initialJobs }: TasksTableProps) => {
	...
	const jobStatusMap = useMap<string, TaskStatus | null>(
		initialJobs.map(({ name }) => [name, null] as const),
	);

	const loadingStates = useMap<
		string,
		{ toggle: boolean; trigger: boolean; interval: boolean }
	>();
	...
	const updateIntervalMutation =
		clientApi.cronJobs.updateJobInterval.useMutation();
