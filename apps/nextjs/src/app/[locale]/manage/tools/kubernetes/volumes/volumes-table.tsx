...
interface VolumesTableComponentProps {
  initialVolumes: RouterOutputs["kubernetes"]["getVolumes"];
}
...
const createColumns = (t: ScopedTranslationFunction<"kubernetes.volumes">): MRT_ColumnDef<KubernetesVolume>[] => [
  ...
  },
  {
    ...
    Cell({ cell }) {
      return cell.row.original.accessModes.map((accessMode) => <div key={nanoid()}>{accessMode}</div>);
    },
  },
  {
...
export function VolumesTable(initialData: VolumesTableComponentProps) {
  ...
  const { data } = clientApi.kubernetes.getVolumes.useQuery(undefined, {
