...
export function ClusterDashboard() {
  ...
  } = clientApi.kubernetes.getCluster.useQuery();

  const {
    data: resourceCountsData,
    isLoading: isResourceCountsLoading,
    isError: isResourceCountsError,
  } = clientApi.kubernetes.getClusterResourceCounts.useQuery();
  ...
  return (
    <Stack bg="var(--mantine-color-body)">
      ...
      <SimpleGrid cols={{ xs: 1, sm: 2, md: 3 }}>
        {isClusterLoading ? (
          Array.from({ length: 3 }).map(() => <Skeleton key={nanoid()} height={65} />)
        ) : (
          <>
      ...
      <SimpleGrid cols={{ xs: 1, sm: 2, md: 3 }}>
        {isClusterLoading
          ? Array.from({ length: 3 }).map(() => <Skeleton key={nanoid()} height={200} />)
      ...
      <SimpleGrid cols={{ xs: 1, sm: 2, md: 3 }}>
        {isResourceCountsLoading
          ? Array.from({ length: 8 }).map(() => <Skeleton key={nanoid()} height={100} />)
