...
interface ServicesTableComponentProps {
  initialServices: RouterOutputs["kubernetes"]["getServices"];
}
...
const createColumns = (t: ScopedTranslationFunction<"kubernetes.services">): MRT_ColumnDef<KubernetesService>[] => [
  ...
  },
  {
    ...
    Cell({ cell }) {
      return cell.row.original.ports?.map((port) => <div key={nanoid()}>{port}</div>);
    },
  },
  {
    ...
    Cell({ cell }) {
      return cell.row.original.targetPorts?.map((targetPort) => <div key={nanoid()}>{targetPort}</div>);
    },
  },
  {
...
export function ServicesTable(initialData: ServicesTableComponentProps) {
  ...
  const { data } = clientApi.kubernetes.getServices.useQuery(undefined, {
