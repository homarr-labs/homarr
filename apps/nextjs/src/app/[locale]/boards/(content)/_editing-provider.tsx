...
export const BoardEditingProvider = ({
  ...
  enabled,
  demoReadOnly,
}: PropsWithChildren<{ enabled: boolean; demoReadOnly: boolean }>) => {
  if (!enabled) return children;
  return <ActiveBoardEditingProvider demoReadOnly={demoReadOnly}>{children}</ActiveBoardEditingProvider>;
};
const ActiveBoardEditingProvider = ({ children, demoReadOnly }: PropsWithChildren<{ demoReadOnly: boolean }>) => {
  const controller = useBoardEditingController(demoReadOnly);
  return <BoardEditingContext.Provider value={controller}>{children}</BoardEditingContext.Provider>;
};
const useBoardEditingController = (demoReadOnly: boolean) => {
  ...
  const enter = useCallback(async () => {
    if (!hasChangeAccess || isPending || isEnteringEditMode) return false;
  ...
  }, [hasChangeAccess, isPending, isEnteringEditMode, isEditMode, open, prepareEditorAsync, t]);

  const toggle = useCallback(async () => {
    if (!hasChangeAccess || isPending || isEnteringEditMode) return;
    if (!isEditMode) return enter();
    if (demoReadOnly) return discardDemoChanges();
    saveBoard(latestBoardRef.current);
  }, [hasChangeAccess, isPending, isEnteringEditMode, isEditMode, enter, demoReadOnly, discardDemoChanges, saveBoard]);
  ...
  usePreventLeaveWithDirty(isEditMode && hasChangeAccess);

  let label = t("enter");
  if (isEditMode) label = t("save");
  if (isEditMode && demoReadOnly) label = t("exit");
  return {
    isEditMode,
    hasChangeAccess,
    label,
