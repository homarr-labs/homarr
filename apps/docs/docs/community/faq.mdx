...
Yes. A [Custom Widget](/docs/management/custom-widgets) can read a public or private HTTP API.

It can show responsive Mantine JSX. It can also use widget options and actions with board permission checks.

Custom JSX v2 is the only Custom Widget format. Write the JSON and JSX in the workbench.

You can also use the AI copy prompt. An MCP agent can validate and preview a definition before it saves the definition.

The workbench reads a pasted AI response. It shows request capabilities and redacted diagnostics.

A template cannot import packages or run arbitrary JavaScript. It cannot call browser `fetch`.

Named requests use the Homarr server. The server applies board permissions and the network policy.

Before installation, examine the origins, methods, permissions, and actions. Use the same review before publication.
