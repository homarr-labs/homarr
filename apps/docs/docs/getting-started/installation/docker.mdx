...
1. Run ``docker rm -f homarr`` to stop and remove the container.
...
1. Obtain the container ID of your Homarr container using ``docker ps -a | grep homarr``.
2. Delete the container using ``docker rm -f <your-container-id>``
