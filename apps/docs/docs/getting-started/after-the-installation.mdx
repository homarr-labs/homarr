...
3. **Connect** combines explicit [`homarr.*` or Homepage Docker labels](/docs/integrations/docker-labels) with image matching. Select several integrations and apps at once, review every URL, and use the service-specific link to open its API-key settings when one is available. If a container has no published port or URL label, add its HTTP or HTTPS address before importing it.
