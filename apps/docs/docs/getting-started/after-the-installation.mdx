...
Hold `Shift` while hovering a widget to expand an advanced preview in place while the rest of the board stays visible
but dimmed. Release `Shift` to close it. You can also focus the widget and press `Shift` + `Enter` to keep its advanced
view open until you close it. App tiles stay compact. If you can change the board, right-click empty grid space to add
an item at that position.
On a touch device, touch and hold empty grid space for the same precise-placement menu.
