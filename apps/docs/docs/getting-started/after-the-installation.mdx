...
Hold `Shift` while hovering a widget for a temporary advanced view; use its pin control to keep it open. You can also
focus the widget and press `Shift` + `Enter` to open it pinned. If you can change the board, right-click empty grid
space to add an item at that position.
