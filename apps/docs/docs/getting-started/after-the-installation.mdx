...
Below 100%, Homarr preserves the widget's readable typography and control density instead of shrinking each widget
independently. Scaling stops at 50%; if the selected layout is still wider than the viewport, the board scrolls
horizontally instead of making tiles and controls unusably small.
