...
The complete canvas always fits the available width without horizontal board scrolling.
Down to 50%, Homarr preserves the widget's readable typography and control density instead of shrinking each widget
independently. Below 50% it stops compensating and scales text and controls together with the board, like zooming out a
page, so tile content keeps its designed proportions instead of overlapping neighboring tiles. Add a narrow responsive
layout — Homarr already creates and reflows the protected Mobile layout — when a board has to stay readable on small screens.
