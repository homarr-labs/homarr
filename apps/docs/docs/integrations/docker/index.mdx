...
For example, two socket proxies can be configured as separate environments:
    ...
    [
      {
        ...
        "name": "Home Docker",
        "kind": "docker",
        "transport": { "type": "tcp", "host": "docker-proxy-home", "port": 2375, "allowInsecure": true },
        "capabilities": ["inventory", "logs", "lifecycle"]
      },
      {
        "id": "lab",
        "name": "Lab Docker",
        "kind": "docker",
        "transport": { "type": "tcp", "host": "docker-proxy-lab", "port": 2375, "allowInsecure": true },
        "capabilities": ["inventory", "logs"]
      }
    ]
```

Homarr must be attached to the Docker network of each proxy, and every configured host must be reachable from the Homarr container.
