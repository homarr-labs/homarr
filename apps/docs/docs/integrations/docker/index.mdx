...
Homarr can also connect to Podman's [Docker-compatible API](https://docs.podman.io/en/latest/markdown/podman-system-service.1.html).
...
Run this compose project as the same user that owns the rootless Podman socket.
...
Its default path is `/run/podman/podman.sock`; mount that path instead.
