...
<IntegrationHeader integration={nextcloudIntegration} categories={["Calendar", "Notifications"]} />

### Widgets & Capabilities

<IntegrationCapabilites items={[{ widget: calendarWidget }, { widget: notificationsWidget }]} />
  ...
  secrets={[
    {
      ...
      header: (
        <Alert type="info">
          Users who use 2FA will have to use Nextcloud's App Password tool to grant Homarr access, as 2FA breaks the
          traditional process. For more information, see [Nextcloud's
          Documentation](https://docs.nextcloud.com/server/stable/admin_manual/configuration_user/authentication.html#app-passwords)
        </Alert>
      ),
    },
  ]}
