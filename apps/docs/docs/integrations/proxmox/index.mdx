  ...
  secrets={[
    {
      header: (
        <Alert type="caution">
          The Proxmox API may not work with Linux PAM standard authentication. It is strongly recommended to use the
          Proxmox VE authentication server realm when creating your API user.
        </Alert>
      ),
      ...
      body: (
        <>
          <span>
            Sometimes you'll also find the <code>api@pve!homarr</code> format, within Homarr you'll have to split it up
            into:
          </span>
          <ul>
            <li>
