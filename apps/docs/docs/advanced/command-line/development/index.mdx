...
A locally built PR image is folded into its remote PR row only when its recorded revision matches the current GitHub head SHA (marked `◆ local`). Starting it prefers that local image without a registry pull. Outdated local builds remain separate rows.
...
| `Enter` or `Space`     | Start local build / pull remote PR image; stop if running                                     |
