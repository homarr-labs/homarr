...
| `HOMARR_AI_OPENROUTER_MODEL`                | OpenAI-served upstream model exposed through `homarr/model`               | `openai/gpt-6-luna`               |
...
SQLite is the default; PostgreSQL is also supported. Existing MySQL installations must [convert to SQLite before upgrading to v2](/docs/advanced/mysql-to-sqlite).
...
trailing slashes when Homarr or the documentation site starts.
