...
- `pnpm dev`: Start Next.js, the task runner, and the WebSocket server as separate development processes.
- `pnpm -F nextjs dev`: Start only the Next.js UI when background services are not needed.
