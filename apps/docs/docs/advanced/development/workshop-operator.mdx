...
The documentation uses Fumadocs search with an index generated during the website build. Search runs in the browser
and needs no hosted search credentials or crawler.
...
See the [Kapa website widget guide](https://docs.kapa.ai/integrations/website-widget/quickstart).
...
it explicitly to this model when upgrading. OpenRouter selects an eligible provider for the model; Workshop does not
pin a provider. Provider fallbacks remain disabled.
...
any model on this provider**. The request's `allow_fallbacks: false` prevents retrying another provider, but does not
pin the initial provider or guarantee OpenAI BYOK billing. Use OpenRouter provider restrictions if required. Store the
OpenAI key in OpenRouter, not in Workshop or Homarr clients.
...
or unavailable key cannot make a non-ZDR route eligible. Other eligible providers may serve the model.
