...
### Icon of the app website

When an app has a URL, the picker offers the icon of that website as its first suggestion. Homarr requests the page the URL points to, reads its `<link rel="icon">` and `<link rel="apple-touch-icon">` declarations and falls back to `/favicon.ico` of the same origin. The suggestion is only requested while the picker is open and it is never selected for you, so an image you picked yourself is never replaced. Nothing is suggested when the site cannot be reached or declares no icon.
