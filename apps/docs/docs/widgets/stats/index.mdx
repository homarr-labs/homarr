...
These are saved snapshots, not live readings. **Right-click → Refresh** fetches new data; the menu shows the age of the oldest snapshot. Failed refreshes keep the previous snapshot; partial failures mark only the affected metrics unavailable.
