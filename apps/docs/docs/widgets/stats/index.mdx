...
These are saved snapshots, not live readings. **Right-click → Refresh** fetches new data; the menu shows the age of the oldest snapshot. Failed refreshes keep the previous values. When an integration uses several endpoints, successful metrics still update and metrics from a failed endpoint display as unavailable.
