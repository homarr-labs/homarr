...
When your permissions allow it, hover the widget, focus within it, or use a touch device to reveal the edit button in the top right. You can also double-click the document to edit it directly.
...
Wraps the current block in a collapsible section with a summary. Use the same control again while inside the section to remove it. Collapsible sections are saved as part of the notebook content and continue to work in read-only mode.
...
The insert-table control opens a grid. Move across the grid to choose the number of columns and rows, then select a cell to insert the table.

![](./img/notebook-table-modal.webp)

Use the delete-table control while the cursor is in a table to remove the entire table.
...
Use the color-cell control to color the background of the selected cells.

The remaining controls edit the table structure:

- Add a column before or after the current column
- Delete the current column
- Add a row before or after the current row
- Delete the current row
- Toggle the first row as a header row
- Toggle the first column as a header column
- Merge selected cells
- Split the current merged cell

Tables remain resizable by dragging their column boundaries.
