...
Both endpoints accept an [API key](api/index.mdx) in the `ApiKey` header, so a scheduled job can
create and restore backups without a browser session:
...
```bash
curl -H 'ApiKey: <id>.<token>' -F 'file=@homarr-backup.zip' http://localhost:7575/api/backup/import
```

The key has to belong to a user with admin permissions. Use
[`homarr api-keys create`](../advanced/command-line/api-keys/) to obtain one without the web interface.
