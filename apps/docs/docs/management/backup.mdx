...
**Management → Tools → Backup**. PostgreSQL installations must use their database-native backup tools.

![The Backup and restore management page](./img/backup.png)

For an existing MySQL installation, [convert to SQLite before upgrading to v2](/docs/advanced/mysql-to-sqlite). The converter produces a database file, not a backup ZIP.
