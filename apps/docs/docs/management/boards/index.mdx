...
- Hold `Shift` while hovering a widget for a temporary advanced view, then use its pin control to keep it open. Focus
  the widget and press `Shift` + `Enter` to open it pinned, or use the maximize control on touch devices.
