...
On each element the position and size of items can be changed separately.
When adding a new layout the position and sizes will automatically be generated depending on the column count.
New widgets use their default size and the first available position on each layout. Adding from an empty grid cell anchors
the widget near that cell on the active layout; Homarr still chooses safe free positions for the other responsive layouts.
...
Once the minimum width of the next layout is reached, the layout will change.
...
- Hold `Shift` while hovering a widget to expand its advanced preview in place while the other widgets remain visible
  but dimmed. Release `Shift` to close it. Focus the widget and press `Shift` + `Enter` to keep the view open until you
  close it, or use the maximize control on touch devices. App tiles stay compact.
