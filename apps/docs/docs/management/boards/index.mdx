...
Mobile and Base can be renamed and their column counts and rails can be changed. You can add removable custom layouts
between their breakpoints.
...
Older or incomplete boards are repaired automatically. Homarr restores missing Mobile/Base layouts, promotes the narrowest
and widest existing layouts to those protected roles, keeps intermediate layouts custom, and normalizes duplicate or
out-of-order breakpoints without discarding saved positions.
...
For other collisions, or when a tile grows into occupied space, nearby tiles are compacted out of the way. A compatible
single-item collision can swap when both placements fit; other collision chains reflow around the move.
...
While edit mode is active, **Add board content** creates a new item, app, integration, or Container on the main canvas.
New content uses the first available position; clicking an empty cell does not place it there, and items cannot be added
directly into a rail or Container from this menu.
...
Same as the home board, you can also set a mobile home board. It is used for devices detected as mobile devices from the
User-Agent header. For the server-wide fallback, configure **Global mobile board** under `Management` > `Settings` >
`Boards`; the selected board must be public. See the [global board defaults](/docs/management/settings#boards) for the
desktop and mobile fallback rules.
