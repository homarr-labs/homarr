...
Widget content is inert in edit mode, so dragging anywhere on the content moves the tile instead of activating the widget; the tile's settings button remains available.
Dropping a tile into the exact slot of an equally-sized tile swaps their positions.
