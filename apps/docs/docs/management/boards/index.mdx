...
import layoutSettingsMobileImage from "./img/layout-settings-mobile.png";
import boardEditModeImage from "./img/board-edit-mode.png";
import addBoardContentMenuImage from "./img/add-board-content-menu.png";
...
<img
  ...
  alt="Board in edit mode with the Save board button and widget editing affordances visible"
  style={{ maxWidth: "100%" }}
/>
...
<img
  src={addBoardContentMenuImage}
  alt="Board edit mode add-content menu showing New item, Add an app, New integration, and New container"
  style={{ maxWidth: "100%" }}
/>
...
On a narrow screen, the same settings page stacks each layout card vertically so the preview and controls remain usable:

<img
  src={layoutSettingsMobileImage}
  alt="Responsive board layout settings stacked in a narrow mobile viewport"
...
Homarr checks a user's mobile home board first, then that user's desktop home board, matching group defaults, and finally
the server-wide mobile or desktop board. Global defaults must be public. See the
