...
In edit mode, nested Container settings controls are placed separately so each menu remains accessible.
...
See [Users and groups](/docs/management/users) and [Server settings](/docs/management/settings#boards).

## Widget previews

The add and edit dialogs use the target board section’s sizing and scale. Large widgets may be shrunk to fit the preview area; their content layout is preserved. Custom CSS that depends on dashboard ancestors may render differently inside the dialog.
