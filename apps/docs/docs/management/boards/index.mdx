...
- **Mobile** starts at `0px`, has no fixed rails, and uses three columns on new boards.
- **Base** starts at `768px` and uses the column count selected when the board is created.

Additional layouts can be added at unique breakpoints. Homarr selects the layout with the highest breakpoint that fits
the current viewport. Each layout stores its own positions and sizes; editing one does not change the others.
