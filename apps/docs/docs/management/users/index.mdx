...
segmented control that lets you pick a single **access level** and, where the category supports it, a separate
**create** checkbox.
...
##### Create is separate from the access level

Creating is a distinct capability, independent of the access level. It can be granted on its own, so you can for
example let members create their own boards without letting them view or modify anyone else's boards. Some access
levels already include the create capability through permission inheritance (marked in the table below); the
checkbox is then disabled and labeled as already included.
...
The access levels below are cumulative, in ascending order, with the category's create capability listed separately.
A level that already includes the create capability is marked:

| Category       | Create capability     | Access levels (ascending)                                                                | Create included at                                   |
| -------------- | --------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Boards         | Create boards         | view all boards &rarr; modify all boards &rarr; full board access                        | full board access                                    |
| Apps           | Create apps           | use all apps &rarr; modify all apps &rarr; full app access                               | modify all apps, full app access                     |
| Integrations   | Create integrations   | use all integrations &rarr; interact with any integration &rarr; full integration access | full integration access                              |
| Search engines | Create search engines | modify all search engines &rarr; full search engine access                               | modify all search engines, full search engine access |
| Medias         | Upload medias         | view all medias &rarr; full media access                                                 | full media access                                    |
| Other          | —                     | view logs                                                                                | —                                                    |
...
- **Apps**: use all apps (add any app to a board); modify all apps; full app access (manage, use and delete any app).
...
- **Viewer**: read-only - view boards and medias, use existing apps and integrations.
- **Editor**: day-to-day management - create and modify boards, apps, integrations and search engines, interact with integrations, and upload medias, without deleting resources or managing access.
...
full board access also allows creating boards), a dimmed **Also grants: …** hint is shown below the category row,
