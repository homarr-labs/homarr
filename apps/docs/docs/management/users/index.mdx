...
Personal control over what appears in the header and where. Drag controls between the Left, Center, and Right
sections, add optional board shortcuts, and choose how the search and logo controls are displayed.

The header can be hidden entirely with the visibility toggle. While hidden, hovering the top edge of the page
reveals it temporarily; clicking inside it pins it open until the page is refreshed or you click elsewhere.

When the header is visible, the **Auto-hide on scroll** toggle makes it slide away while scrolling down and
reappear on scroll-up or near the top of the page. It has no effect while the header itself is hidden.
...
The group-name form expands above the group list and can be closed without leaving the page.
...
Select the create-invitation button. The expiration form expands above the invitation table instead of opening a
dialog or navigating to another page:
...
Next, click `Create` and the same section shows the ID, Token and a link that can be copied:
...
Use **Copy** to copy the URL, then close the section when you are finished.
...
The first click changes the action to **Confirm**; the second click deletes the invitation without opening a dialog.
