...
8. Change the definition with `customWidget_templatePatch` or `customWidget_update`.
9. Set a supplied credential with `customWidget_secretSet`.
10. Otherwise, call `customWidget_secretRequestUser` to create a ten-minute Homarr entry URL.
11. Call `customWidget_create`.
12. Repeat this procedure for the next Custom Widget.

The agent receives the credential request status. The agent does not receive the plaintext credential.
...
- `customWidget_secretRequestUser`
