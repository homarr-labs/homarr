...
Supported types are discovered from Homarr's integration registry and use the integration's native HTTP authentication
adapter. Integrations without this capability are rejected. The built-in Assistant's default mutation approval applies to
this tool, including GET calls.
