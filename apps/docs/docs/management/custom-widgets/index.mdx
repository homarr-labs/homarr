...
Custom widget definitions, endpoint details, authentication settings, and secrets are managed by administrators.
Board editors can select an enabled definition without receiving its endpoint or credentials, while board viewers only
receive the rendered widget data they are allowed to view. **Action Button** requests are administrator-only.

Custom API requests accept HTTP and HTTPS endpoints without URL-embedded credentials. Redirects are rejected, requests
time out after 10 seconds, and responses are limited to 2 MiB. Detailed upstream errors stay in the server logs; the
dashboard only shows a generic failure.
