...
A **Custom Widget** is a validated definition with a safe JSX template, optional settings, and optional HTTP sources,
queries and actions. Existing definitions use `homarr-custom-widget-v2`; advanced composition and native capabilities use
`homarr-custom-widget-v3`. Static widgets need no API source.
...
Use **Import from Workshop** while adding board content or import a supported v2 or v3 JSON definition directly. Review the source,
