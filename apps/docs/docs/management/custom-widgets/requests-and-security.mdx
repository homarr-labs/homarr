...
HTTP sources and saved integrations can reach any IPv4 or IPv6 destination accessible to Homarr, including loopback,
private, link-local, and tailnet addresses. Legacy `networkScope` fields no longer restrict destinations.
...
Every request uses Homarr's existing server executor: same-origin source enforcement, DNS pinning and redirect validation, board permissions, rate and request limits, body and response limits, timeouts, and encrypted credential injection. Custom proxy, forwarding, and `sec-*` headers are accepted. Authentication, cookies, host, and transport-managed headers remain reserved. URL fragments are discarded before sending requests.
