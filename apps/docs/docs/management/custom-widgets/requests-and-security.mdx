...
Widgets can use empty `sources` and `requests` objects when they do not need an API. Connected widgets may use a source
named `default` or explicitly name the source on every request. Extra keyed sources are available for widgets
...
Use `private` for LAN addresses, Docker service names, and Tailscale's `100.64.0.0/10` range. Use `loopback` for
`127.0.0.1` or `::1`. Requests run from Homarr: in Docker, loopback addresses point to the Homarr container, while a
service name such as `http://sonarr:8989` must resolve on its Docker network. Public sources cannot reach private or
loopback addresses. Cloud metadata, link-local, multicast, and other reserved targets remain blocked.

## Named requests

Requests are keyed by ID. With a source named `default`, a load query can be as small as:
