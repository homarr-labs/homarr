...
Available types come from Homarr's integration registry. Saved credentials are applied by the integration's authentication
contract, including services that require a login session. **iCalendar feeds (iCal)** are calendar URLs rather than arbitrary
service APIs, and **TrueNAS uses a WebSocket API**; neither appears as a generic HTTP source. qBittorrent requires a saved
API key and qBittorrent 5.2.0 or newer; saved username/password credentials cannot authenticate generic requests. UniFi
generic requests require an HTTPS saved URL with the controller's actual port.
