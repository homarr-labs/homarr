...
HTTP widgets often use a source named `default`. Every request must reference an existing source; an omitted request source
resolves to `default`. Static and native-only widgets can use empty `sources` and `requests`. Extra keyed sources let widgets
combine APIs. Source IDs are case-insensitively unique. Base URLs must use HTTP(S), contain no embedded credentials,
...
Source URLs are fetched by the Homarr server. In a container, `127.0.0.1` refers to that container; Docker service names such
as `sonarr` must resolve from Homarr. LAN addresses and loopback services are supported with the appropriate explicit scope.
The workbench suggests a likely local scope from the hostname; server DNS and redirect checks remain authoritative.
Browser navigation and app embeds use browser-reachable URLs instead.
