...
Every request uses Homarr's existing server executor: same-origin source enforcement, DNS pinning and redirect validation, board permissions, rate and request limits, body and response limits, timeouts, and encrypted credential injection. Static headers cannot override authentication, cookies, host, or transport-managed headers. Legacy `networkScope` fields are ignored.
