...
![The Trusted certificates management page](../img/certificates.png)

![Trusted certificates page listing a certificate's subject, filename, and expiry](./img/screenshot.png)
...
![Add certificate button on the Trusted certificates page](./img/add-button.png)
...
![Certificate card with subject, filename, expiry date, and delete action](./img/card.png)
...
For the default Proxmox VE cluster certificates, trust the cluster CA from `/etc/pve/pve-root-ca.pem` on any node.
See [Proxmox certificate management](https://pve.proxmox.com/wiki/Certificate_Management).
