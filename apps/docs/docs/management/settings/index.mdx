...
**Use automatic mobile layout** controls how every board is rendered on phones and small tablets:

- Enabled: Homarr squeezes the normal widgets into a generated, read-only two-column grid. Widget content is not replaced or simplified.
- Disabled: Homarr uses each board's saved responsive layouts and permits layout editing on mobile.

The setting is enabled by default for new instances. During upgrade, Homarr leaves it disabled when existing custom responsive layouts are detected. Saved responsive layouts are preserved while automatic mode is enabled.
