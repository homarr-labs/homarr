...
export const AppWidget = ({ name, label, className }: AppWidgetProps) => {
  ...
  return (
    <WidgetCard width={1} className={clsx("app-card text-center", className)}>
      <span className="absolute top-2 inset-x-1 text-sm font-bold leading-4">{label ?? app.name}</span>
      <img src={app.iconUrl} className="absolute bottom-0 size-24 object-contain scale-[0.6]" alt="" />
