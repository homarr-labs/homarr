...
export const getRuntimeWorkshopWebUrl = (configuredUrl: string): string => {
  if (typeof window === "undefined") return configuredUrl;
  return window.homarrRuntimeConfig?.workshopWebUrl || configuredUrl || `${window.location.origin}/workshop`;
};
