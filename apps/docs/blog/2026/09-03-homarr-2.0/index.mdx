...
authors:
  - ajnart
tags: [homarr, update, version, assistant, custom widgets, dashboards, docker, mcp]
description: Homarr 2.0 introduces Custom Widgets and Workshop, rebuilt board editing, Assistant and MCP tooling that turns Homarr into the front door to your homelab, responsive layouts, onboarding, and more.
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="metadata"
    poster={require("./img/homarr-v2-overview.webp").default.src}
    width={1920}
    height={1080}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Homarr 2.0 feature overview."
  >
    <source src="/media/homarr-2.0/homarr-v2-overview.mp4" type="video/mp4" />
  </video>
...
<Alert type="warning" title="Workshop is moderated">
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/drag-and-resize-poster.webp").default.src}
    width={1770}
    height={706}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Move and resize a tile with the new drag-and-drop system."
  >
    <source src="/media/homarr-2.0/drag-and-resize.mp4" type="video/mp4" />
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/containers-poster.webp").default.src}
    width={1770}
    height={716}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Move, collapse and reopen a Container without rearranging its contents."
  >
    <source src="/media/homarr-2.0/containers.mp4" type="video/mp4" />
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/multi-select-poster.webp").default.src}
    width={1770}
    height={716}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Command-click two apps, then move them into the same Container."
  >
    <source src="/media/homarr-2.0/multi-select.mp4" type="video/mp4" />
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/sidebar-settings-poster.webp").default.src}
    width={1600}
    height={700}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Enable the right sidebar and set its width."
  >
    <source src="/media/homarr-2.0/sidebar-settings.mp4" type="video/mp4" />
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/fixed-sidebar-poster.webp").default.src}
    width={1580}
    height={640}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="The board scrolls; the app shortcuts stay in place."
  >
    <source src="/media/homarr-2.0/fixed-sidebar.mp4" type="video/mp4" />
...
The most powerful one gets its own section.

## The front door to your homelab

Homarr already knows how to reach your services: their URLs, API keys and auth headers. v2 adds one API endpoint that
lets you use them without ever seeing them.

`POST /api/integrations/request` calls **any HTTP endpoint** on a saved integration. It is also available as tRPC
`integration.request` and MCP `integration_request`. You send the integration, a method and a path, authenticated with
your Homarr API key or session. Homarr checks your access, adds the credentials it stores and hands back the service's
answer:

```mermaid
sequenceDiagram
    participant C as Custom Widget or agent
    participant H as Homarr
    participant S as Sonarr
    C->>H: POST /api/integrations/request (sonarr, GET /api/v3/queue) with your API key
    H->>H: Authenticate you, check full integration access, keep the path on Sonarr's URL
    H->>S: GET /api/v3/queue with the saved Sonarr API key
    S-->>H: 200 and JSON
    H-->>C: The response, with stored secrets redacted
```

The secrets stay in Homarr. API keys, passwords and tokens are stored encrypted, no API returns them, and Homarr redacts
them from responses. If a service's API can do something, you can do it through Homarr, even when Homarr has no button
for it:

- **Custom Widgets** bind a saved integration as a [source](/docs/management/integrations#beyond-native-widgets) and call
  its endpoints, so you never paste credentials into widget code.
- **Agents** act on your behalf through MCP. The agent reads the service's official API docs, tells you which integration
  it will use and what the call changes, then reports the result.

It stays inside the lines:

- Paths stay within the saved integration URL, and redirects are rejected.
- Every request needs **full integration access**. `DELETE` needs explicit confirmation, and in Assistant every request,
  even a `GET`, waits for your approval.

### One prompt, seven services

We asked a local model, through Homarr's MCP server:

> Could you install a notification provider and then set it up everywhere it's supported in my installed integrations?

It checked the Docker endpoint, asked which provider to use (it recommended ntfy), deployed ntfy and reported back:

| Service     | How                                                                       | Verified                                   |
| ----------- | ------------------------------------------------------------------------- | ------------------------------------------ |
| Sonarr      | Native ntfy connection for grabs, downloads, upgrades and health          | Test fired ✓                               |
| Radarr      | Native ntfy connection, plus movie added                                  | Test fired ✓                               |
| Seerr       | Webhook with a JSON payload for readable titles                           | Fires on request, approve, available, fail |
| SABnzbd     | 5.1.3 has no ntfy support, so a completion script on the default category | Dry run ✓                                  |
| qBittorrent | `[AutoRun]` completion hook in `qBittorrent.conf`                         | Persisted after restart                    |
| Bazarr      | ntfy provider enabled for subtitle events                                 | Restarted ✓                                |
| Beszel      | Webhook with status, CPU, memory and disk alerts on all eight systems     | 29 alerts created                          |

Two test notifications later, the phone buzzed. It cost $0: Qwen3.8-27B ran locally through MLX. The only side effect
was warm feet.

See [authenticated integration requests](/docs/management/mcp#authenticated-integration-requests) and the
[API contract](/docs/management/api#call-any-integration-api-endpoint) for selectors, limits and the services that
cannot use it.
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/board-switcher-poster.webp").default.src}
    width={1920}
    height={778}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Open the switcher, filter by name and press Enter to open a board."
  >
    <source src="/media/homarr-2.0/board-switcher.mp4" type="video/mp4" />
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/timer-poster.webp").default.src}
    width={950}
    height={500}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Start, pause and reset Timer without leaving the board."
  >
    <source src="/media/homarr-2.0/timer.mp4" type="video/mp4" />
...
<figure style={{ margin: "1.5rem 0" }}>
  <video
    ...
    playsInline
    preload="none"
    poster={require("./img/advanced-widget-poster.webp").default.src}
    width={864}
    height={600}
    style={{ width: "100%", height: "auto", borderRadius: "0.5rem" }}
    aria-label="Hold Shift for one second over Downloads to open its advanced view."
  >
    <source src="/media/homarr-2.0/advanced-widget.mp4" type="video/mp4" />
...
[Hostinger now offers a one-click Docker deployment of Homarr](https://www.hostg.xyz/aff_c?offer_id=815&aff_id=244277&url_id=6861) on its VPS plans. They maintain the deployment template, and you manage the resulting VPS and Homarr instance. Our new [Hostinger installation guide](/docs/getting-started/installation/hostinger) walks through deployment and first setup.

We deploy the Homarr Workshop and the compiled documentation website on the VPS provided by Hostinger. Thank you for hosting this part of Homarr's infrastructure. We have an affiliate partnership as well: purchases through a referral link may support Homarr.
