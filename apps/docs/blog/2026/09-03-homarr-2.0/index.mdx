...
**Our most powerful MCP tool, `integration_request`, goes beyond Homarr's native features.** If a supported integration's
API exposes an endpoint, your agent can test it using saved credentials, then expose its data or actions through a
Custom Widget. You do not have to wait for a native widget. See [Beyond native widgets](/docs/management/integrations#beyond-native-widgets).
