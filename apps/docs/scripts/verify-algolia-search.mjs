...
const DOCSEARCH_FACETS = ["docusaurus_tag", "language", "lang", "version", "type"];
...
async function verifyDocsearchIndex(index, hitCount) {
  ...
  for (const searchCase of SEARCH_CASES) {
    ...
    const staleUrls = (result.hits ?? []).map((hit) => hit.url).filter((url) => STALE_URL_PATTERN.test(url));
...
async function verifyMarkdownIndex(index, hitCount) {
  ...
  ok =
    reportLine((coolify.nbHits ?? 0) >= 1, `markdown index "${index}" returns coolify (${coolify.nbHits ?? 0} hits)`) &&
    ok;

  const sample = coolify.hits?.[0];
  ok = reportLine(Boolean(sample?.text?.length), `markdown records include chunked text content`) && ok;
...
async function main() {
  ...
  const docsearchOk = await verifyDocsearchIndex(docsearch.index, docsearch.hitCount);

  console.log("");
  const markdown = await resolveMarkdownIndex();
  console.log(`Using markdown index: ${markdown.index}\n`);
  const markdownOk = await verifyMarkdownIndex(markdown.index, markdown.hitCount);
