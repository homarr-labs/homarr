...
The draft PR defect review additionally found and corrected the compact Beszel Disk metric bypassing shared label/progress rules, and Firewall rings expanding before the compact width/height thresholds ended. Follow-up captures of the Beszel and security families each produced 120 ready widget images plus three ready boards. Firewall's 30 permutations were inspected; a temporary Disk-enabled 1×1 configuration verified its percentage at all three profiles and was restored afterward. Those diagnostic captures are separate from the matched matrix and do not constitute a new complete comparison run.

Capture tooling now rejects partial recaptures with mixed run/source/fixture identities before overwriting evidence, preserves unselected boards, and requires an explicit expected widget count for direct captures. Rejection smoke checks, widget type checking, and six existing Beszel/Firewall checks passed; the review agent's bounded re-review found no residual defects in these fixes.
