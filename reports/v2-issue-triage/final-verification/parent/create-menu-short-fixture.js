...
await call('board.createBoard',{name:'menu-short',columnCount:12,isPublic:true});
const response=await fetch('/api/trpc/board.getBoardByName?input='+encodeURIComponent(JSON.stringify({json:{name:'menu-short'}})));
...
for(let depth=0;depth<4;depth++){const id=board.id+'-depth-'+depth;board.sections.push({id,kind:'container',options:{...options,title:'Depth '+depth,showLabel:false,collapsible:true},collapsed:false,layouts:board.layouts.map(l=>({layoutId:l.id,parentSectionId:parent,xOffset:0,yOffset:0,width:1,height:1}))});parent=id;}
