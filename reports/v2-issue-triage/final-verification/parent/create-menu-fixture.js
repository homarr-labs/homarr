...
await call('board.createBoard',{name:'menu-regression',columnCount:12,isPublic:true});
const response=await fetch('/api/trpc/board.getBoardByName?input='+encodeURIComponent(JSON.stringify({json:{name:'menu-regression'}})));
...
for(let depth=0;depth<4;depth++){const id='menu-depth-'+depth;board.sections.push({id,kind:'container',options:{...options,title:'Depth '+depth,showLabel:false,collapsible:true},collapsed:false,layouts:board.layouts.map(l=>({layoutId:l.id,parentSectionId:parent,xOffset:0,yOffset:0,width:l.role==='mobile'?3:8,height:8-depth}))});parent=id;}
